...
void measureTask(void *pvParameters) {
  ...
  while (1) {
    ...
    for (auto i = 0; i < 3; i++)
      sample[i] =
          ((float)offsetRawData[i] - rawData[i]) * lsm6dso.getSensitivity();
...
void oledDisplayTask(void *pvParameters) {
  ...
  while (1) {
    ...
    // キューから値を取り出す
    if (xQueueReceive(displayIntensityQueue, &rawInt, 0) == pdFALSE &&
        rawInt == NAN)
      continue;
    ...
    if (millis() - maxIntensityAt > 60 * 10 * 1000 ||
    ...
    display.displayIntensity(latestIntensity, rawInt,
                             processor->calcStdDev() <= 0.05, maxIntensity);
  }
}
...
void httpTask(void *pvParameters) {
  ...
  server.on("/hwinfo", HTTP_GET, [](AsyncWebServerRequest *request) {
    char buffer[128];
    sprintf(buffer, "XSHWI,1,%s,%s,%s,%s,%f", APP_NAME, APP_VERSION,
            SEISMOMETER_DEVICE_NAME, SEISMOMETER_SENSOR_NAME,
            SEISMOMETER_ADC_NAME, SEISMOMETER_ADC_STEP);
  ...
  while (1) {
    ...
    // キューから値を取り出す
    float rawIntensity;
    if (xQueueReceive(webSocketIntensityQueue, &rawIntensity, 0) == pdFALSE &&
        rawIntensity == NAN)
      continue;
    else
    ...
    float acc[3];
    if (xQueueReceive(webSocketSensorDataQueue, &acc, 0) == pdFALSE &&
        acc[0] == NAN) {
      continue;
    } else {
      doc["ACC"]["X"] = acc[0];
...
void setup() {
  ...
  processor = new IntensityProcessor(
      ...
      [](float rawInt) {
        printNmea("XSINT,%.3f,%.2f", -1.0,
                  processor->getIsStable() ? rawInt : NAN);
  ...
  xTaskCreatePinnedToCore(oledDisplayTask, "OLEDDisplay", 4096, NULL, 5, NULL,
                          0x01);
  xTaskCreatePinnedToCore(serialCommandTask, "Serial", 4096, NULL, 5, NULL,
                          0x01);
  ...
  xTaskCreateAffinitySet(oledDisplayTask, "OLEDDisplay", 2048, NULL, 5, 0x01,
                         NULL);
  xTaskCreateAffinitySet(serialCommandTask, "Serial", 4096, NULL, 5, 0x01,
                         NULL);
#endif
